...
export function exportMarkdown(state: ProjectState): string {
  ...
  if (state.justification) {
    md += `### Justification\n\n`;
    if (state.justification.pros.length > 0) {
      md += `**Pros:**\n`;
      state.justification.pros.forEach(pro => md += `- ${pro}\n`);
      md += `\n`;
    }
    if (state.justification.cons.length > 0) {
      md += `**Cons:**\n`;
      state.justification.cons.forEach(con => md += `- ${con}\n`);
      md += `\n`;
    }
    if (state.justification.alternatives.length > 0) {
      md += `**Alternatives:**\n`;
      state.justification.alternatives.forEach(alt => md += `- ${alt}\n`);
      md += `\n`;
    }
  }
